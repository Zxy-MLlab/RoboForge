"""Register the sealed v019 LIBERO-Spatial transfer and frontier outcomes."""
...
RUN_ROOT = ROOT / "runs" / "coding_harness" / "libero_spatial_v019_state12_full"
...
def main():
    ...
    assets = [base(
        asset_id="experience.frozen-libero-spatial-v019-state12-frontier.v1",
        kind="experience", name="Frozen LIBERO-Spatial v019 state-12 transfer", version="1",
        status="frozen_candidate", evidence=str(RUN_ROOT / "summary.json"),
        run_root=str(RUN_ROOT), controller_id=summary["controller_id"], state=12,
        ...
        known_failures=["tasks 0,5,7,8,9 unresolved", "higher safety height did not improve aggregate score"],
    )]
    for task in range(10):
        run = RUN_ROOT / "runs" / f"task{task}_state12_seed7"
        ...
        reason = None if success else ("attachment_not_verified" if not result.get("attachment_verified") else "placement_not_verified")
        assets.append(base(
            asset_id=f"{'experience' if success else 'frontier.failure'}.libero-spatial-task{task}-v019-state12.v1",
            kind="experience" if success else "frontier_failure",
            name=f"LIBERO-Spatial task {task} v019 state 12 {'success' if success else 'unresolved'}",
            ...
            evaluator_calls=scorer.get("evaluator_calls"), failure_reason=reason,
        ))
    for asset in assets:
